...
onMounted(async () => {
  ...
  if (store.currentPlan) {
    const p = store.currentPlan
    const startParts = p.start_date.split('-').map(Number)
    if (startParts.length === 3) {
      const js = jalaali.toJalaali(startParts[0]!, startParts[1]!, startParts[2]!)
      form.startDateShamsi = `${js.jy}/${String(js.jm).padStart(2, '0')}/${String(js.jd).padStart(2, '0')}`
    }
    if (p.end_date) {
      const endParts = p.end_date.split('-').map(Number)
      if (endParts.length === 3) {
        const je = jalaali.toJalaali(endParts[0]!, endParts[1]!, endParts[2]!)
...
async function onSubmit() {
  ...
  try {
    await store.updatePlanHeader(planId, {
      start_date: startDate,
      end_date: endDate ?? undefined,
      notes: form.notes || undefined,
      daily_water_target_ml: form.waterMl || undefined,
    })
    await router.push(`/nutritionist/clients/${clientId}/plans/${planId}`)
  }
  catch (e: unknown) {
    const err = e as { data?: { error?: string } }
    error.value = err.data?.error ?? store.error ?? 'خطا در ذخیره تغییرات'
  }
  finally {
...
<template>
  <div class="min-h-screen bg-gray-50" dir="rtl">
    ...
    <div v-if="store.loading" class="p-4 space-y-3">
      <div v-for="i in 3" :key="i" class="bg-white rounded-2xl p-4 h-16 animate-pulse" />
    </div>
    <!-- Read-only notice for non-draft plans (T-03-05-B) -->
    <div v-else-if="store.currentPlan?.status !== 'draft'" class="p-4">
      <p class="text-center text-gray-500 py-12">فقط برنامه‌های پیش‌نویس قابل ویرایش هستند</p>
    </div>
    <form v-else class="p-4 space-y-4 max-w-lg mx-auto" @submit.prevent="onSubmit">
