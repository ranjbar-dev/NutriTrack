...
async function load() {
  loading.value = true
  try {
    items.value = await fetchDomain('exercise')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="px-4 py-4 pb-20">
      <h1 class="mb-3 text-lg font-bold text-gray-800 text-start">تمرین‌ها</h1>
      ...
      <div v-else class="space-y-2">
        <div v-for="item in items" :key="item.id" class="rounded-2xl bg-white p-4 shadow-sm">
          <div class="flex items-center justify-between gap-2">
            <div class="flex-1 min-w-0 text-start"><p class='text-sm text-gray-800'>{{ item.exercise_name }}</p><p class='mt-1 text-xs text-gray-500'>{{ item.duration_minutes }} دقیقه<span v-if='item.calories_burned'> · {{ item.calories_burned }} کالری</span></p></div>
