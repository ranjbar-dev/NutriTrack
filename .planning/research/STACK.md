...
go mod init github.com/ranjbar-dev/nutritrack
...
npx nuxi@latest init nutritrack-frontend
cd nutritrack-frontend

# Core dependencies
npm install pinia vazirmatn jalaali-js dexie chart.js vue-chartjs
...
services:
  traefik:
    image: traefik:v3.4
    # ... Let's Encrypt + Docker provider config

  api:
    build:
      context: ./backend
      dockerfile: Dockerfile
    # Multi-stage Go build → ~20MB image

  frontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile
    # Node.js for Nuxt SSR
...
/* Custom Persian theme tokens */
@theme {
  --font-family-sans: 'Vazirmatn', system-ui, sans-serif;
}
```
...
```html
<!-- Usage: logical properties auto-flip for RTL -->
<div class="ms-4 me-2 ps-3 pe-1 text-start">
  <!-- ms-4 = margin-inline-start → margin-right in RTL -->
...
config.MaxConns = 20          // Sufficient for ~500 concurrent users
config.MinConns = 5
