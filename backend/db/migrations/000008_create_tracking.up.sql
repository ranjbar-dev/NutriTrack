...
-- water_logs: multiple entries per day allowed
CREATE TABLE water_logs (
    ...
    logged_time TIME,
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_water_logs_local_id UNIQUE (local_id)
);
CREATE INDEX idx_water_logs_client_date ON water_logs (client_id, date);

-- sleep_logs: one record per (client, date); upserts on conflict
CREATE TABLE sleep_logs (
    ...
    created_at TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_sleep_logs_local_id    UNIQUE (local_id),
...
-- exercise_logs: multiple entries per day allowed
CREATE TABLE exercise_logs (
    ...
    notes            TEXT,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_exercise_logs_local_id UNIQUE (local_id)
);
CREATE INDEX idx_exercise_logs_client_date ON exercise_logs (client_id, date);

-- medication_logs: links to prescribed medications or self-reported
CREATE TABLE medication_logs (
    ...
    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_medication_logs_local_id UNIQUE (local_id)
);
CREATE INDEX idx_medication_logs_client_date ON medication_logs (client_id, date);

-- body_measurements: one record per (client, date); upserts on conflict
CREATE TABLE body_measurements (
    ...
    created_at  TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_body_measurements_local_id    UNIQUE (local_id),
...
-- lab_results: file upload or external link; at least one required (validated at service layer)
