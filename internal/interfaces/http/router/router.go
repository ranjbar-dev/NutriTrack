...
func New(container *bootstrap.Container) *gin.Engine {
	if container.Cfg.App.Env == "production" {
	...
	r.Use(middleware.CORS(container.Cfg.App.CORSAllowedOrigins))
	...
	authGroup := v1.Group("/auth")
	{
		...
		authGroup.POST("/otp/send", middleware.RateLimitByIP(container.RateLimiter, 60), authHandler.SendOTP)
